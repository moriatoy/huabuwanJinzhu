...
function loadMyEssay() {		
	...
	function init(pageNo){
		...
		$("#thislist").html("");
		$.ajax({
			url: urlcore + "/api/role/findList?pageNo="+pageNo+"&pageSize="+pageSize,
			type: "get",
			dataType: 'json',
...
'                                        <a hidden="hidden" name="详情" class="" href="myCatalogue_list.html?roleId='+n.id+'" >详情</a>'+
'                                        <a hidden="hidden" name="修改" class="" href="role_add.html?roleId='+n.id+'&roleName='+escape(n.title)+'" >修改</a>'+
'                                        <a hidden="hidden" name="删除" class="" href="#" onclick="deleteeach('+n.id+')">删除</a>'+
'                                    </td>'+
'                                </tr>';
...
function deleteeach(id){
	$.ajax({
        url: urlcore + "/api/admin/deleteRole?id="+id,
        type: "get",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        success:function(data){
