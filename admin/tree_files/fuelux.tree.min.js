	...
	b.prototype = {
			...
			this.options.dataSource.data(f.data(), function(g) {
				...
				a.each(g.data, function(h, j) {
					...
					if(j.type === "folder") {
						...
						i.find(".tree-folder-name").html(j.name).attr("data", j.data).attr("id", j.id);
						...
						if(j.type === "item") {
							...
							i.find(".tree-item-name").html(j.name).attr("name", j.data);
							i.data(j);
		...
		selectItem: function(e) {
			...
			if(d.hasClass("tree-selected")) {
				var thisId = $(d.find("div")).attr("name");
				removeByValue(myArray,thisId);
