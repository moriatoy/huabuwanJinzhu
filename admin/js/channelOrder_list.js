...
//		findMyCatalogue();
	...
	function init(pageNo) {
		$("#thislist").html("");
		$.ajax({
			url: urlcore + "/api/loanOrder/channelSelectLoanOrder?gmtDatetime=" + limitPayTime + "&userName=" + name + "&phone=" + phoneNumber + "&currentPage=" + pageNo+"&status="+status,
			type: "get",
			async: 'false',
			dataType: 'json',
			...
			success: function(data) {
				if(data.success == true) {
					$.each(data.data.pageDto.list, function(i, n) {
						...
						var phone = hidePhone(n.user.phone);
						var thislist =
